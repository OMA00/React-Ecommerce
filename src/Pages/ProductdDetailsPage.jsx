...
const ProductDetailsPage = () => {
  ...
  if (error)
    return (
      <div
        className="text-red-500 text-center h-64 px-20 flex justify-center items-center
  ...
  const product =
    getProductById(sku) || products.find(() => product.sku === sku);

  if (!product) {
    return (
      <div
        className="text-red-500 text-center h-64 px-20 flex justify-center items-center font-bold text-xl
      capitalize"
      >
  ...
  const images = useMemo(() => {
    ...
    const fallbacks = [];
    for (let i = 0; i < desired - provided.length; ) {
      // use lighweight stock image as our fallback ( vary with sig)
      fallbacks.push({
    ...
    // Only recompute when the product's relevant fields *--->>>>Subject to change>>>>>>
